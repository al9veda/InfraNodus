...
CypherQuery.addStatement = function(user,concepts,statement,contexts,mentions,gapscan,callback) {
    ...
    if (typeof mentions !== 'undefined' && mentions.length > 0) {
        createFirstMentionQuery += 'MERGE (' + convertMention(mentions[0]) + ':Concept ' + '{name:"' + mentions[0] + '"}) ON CREATE SET ' + convertMention(mentions[0]) + '.timestamp="' + timestamp + '", ' + convertMention(mentions[0]) +  '.uid="' + ment_uid + '" ';
