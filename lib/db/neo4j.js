...
CypherQuery.addStatement = function(user,concepts,statement,context,callback) {
    ...
    createContext = 'MERGE (c:Context ' + '{uid:"' + context.uid + '"}) ON CREATE SET c.timestamp="' + timestamp + '" MERGE c-[c_u:BY{timestamp:"' + timestamp + '"}]->u ON CREATE SET c_u.timestamp="' + timestamp + '" ';
