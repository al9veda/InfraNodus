...
CypherQuery.addStatement = function(user,concepts,statement,contexts,gapscan,callback) {
    ...
    createEdgesQuery = 'CREATE ' + concepts[0] + '-[:BY {timestamp:"' + timestamp + '",statement:"' + statement.uid + '"}]->u ';

    for (var indx = 0; indx < contexts.length; ++ indx) {
        createEdgesQuery += 'CREATE ' + concepts[0] + '-[:OF {context:"' + contexts[indx].uid + '",user:"' + user.uid + '",timestamp:"' + timestamp + '"}]->s  CREATE ' + concepts[0] + '-[:AT {user:"' + user.uid + '",timestamp:"' + timestamp + '",statement:"' + statement.uid + '"}]->'+'c_' + contexts[indx].name+' ';
    }
    ...
    for (var index = 1; index < concepts.length; ++ index) {
        ...
        if (concepts[minusOne] !== concepts[index]) {
            ...
            for (var indx = 0; indx < contexts.length; ++ indx) {
                ...
                createEdgesQuery += 'CREATE ' + concepts[minusOne] +'-[:TO {context:"' + contexts[indx].uid + '",statement:"' + statement.uid + '",user:"' + user.uid + '",timestamp:"'+(timestamp+index)+'",uid:"' + edge_uid + '",gapscan:"2",weight:"' + narrativeScanWeight + '"}]->' + concepts[index] + ' ';
            ...
            for (var indexGap = leftGap; indexGap < (index - 1); ++indexGap) {
                ...
                if (concepts[indexGap] !== concepts[index]) {
                    ...
                    for (var indx = 0; indx < contexts.length; ++ indx) {
                        ...
                        // Make query
                        createEdgesQuery += 'CREATE ' + concepts[indexGap] +'-[:TO {context:"' + contexts[indx].uid + '",statement:"' + statement.uid + '",user:"' + user.uid + '",timestamp:"'+(timestamp+index)+'",uid:"' + edge_uid + '",gapscan:"' + scanGap + '",weight:"' + weightInGap + '"}]->' + concepts[index] + ' ';
        ...
        createEdgesQuery += 'CREATE ' + concepts[index]  + '-[:BY {timestamp:"' +(timestamp+index)+ '",statement:"' + statement.uid + '"}]->u ';

        for (var indx = 0; indx < contexts.length; ++ indx) {
            createEdgesQuery += 'CREATE ' + concepts[index] + '-[:OF {context:"' + contexts[indx].uid + '",user:"' + user.uid + '",timestamp:"'+(timestamp+index)+'"}]->s CREATE ' + concepts[index] + '-[:AT {user:"' + user.uid + '",timestamp:"' + (timestamp+index) + '",statement:"' + statement.uid + '"}]->'+ 'c_' + contexts[indx].name +' ';
        }
