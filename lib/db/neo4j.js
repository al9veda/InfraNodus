...
CypherQuery.addStatement = function(user,concepts,statement,contexts,mentions,gapscan,callback) {
    ...
    if (typeof mentions !== 'undefined' && mentions.length > 0) {
        createMentionsQuery = 'MERGE (' + convertMention(mentions[0]) + ':Concept ' + '{name:"' + mentions[0] + '"}) ON CREATE SET ' + convertMention(mentions[0]) + '.timestamp="' + timestamp + '", ' + convertMention(mentions[0]) +  '.uid="' + ment_uid + '" ';
        mentions_added.push(mentions[0]);
    }
    ...
    for (var indx = 0; indx < contexts.length; ++ indx) {
        ...
        createEdgesQuery += 'CREATE ' + convertName(concepts[0]) + '-[:OF {context:"' + contexts[indx].uid + '",user:"' + user.uid + '",timestamp:"' + timestamp + '"}]->s  CREATE ' + convertName(concepts[0]) + '-[:AT {user:"' + user.uid + '",timestamp:"' + timestamp + '",statement:"' + statement.uid + '"}]->'+'c_' + contexts[indx].name+' ';
    ...
    if (concepts.length == 1) {
        ...
        for (var indx = 0; indx < contexts.length; ++ indx) {
            ...
            createEdgesQuery += 'CREATE ' + convertName(concepts[0]) +'-[:TO {context:"' + contexts[indx].uid + '",statement:"' + statement.uid + '",user:"' + user.uid + '",timestamp:"'+(timestamp)+'",uid:"' + edge_uid + '",gapscan:"2",weight:"' + narrativeScanWeight + '"}]->' + convertName(concepts[0]) + ' ';

            edge_uid = uuid.v1();
        }
    }

    // If the statement has only one mention, add an edge where it links to itself, so it can be shown in the graph along with its context
    if (mentions.length == 1) {
        ...
        for (var indx = 0; indx < contexts.length; ++ indx) {
            ...
            createMentionsQuery += 'CREATE ' + convertMention(mentions[0]) +'-[:TO {context:"' + contexts[indx].uid + '",statement:"' + statement.uid + '",user:"' + user.uid + '",timestamp:"'+(timestamp)+'",uid:"' + ment_uid + '",gapscan:"1",weight:"' + narrativeScanWeight + '"}]->' + convertMention(mentions[0]) + ' ';
    ...
    for (var index = 1; index < concepts.length; ++ index) {
        ...
        for (var indx = 0; indx < contexts.length; ++ indx) {
            ...
            createEdgesQuery += 'CREATE ' + convertName(concepts[index]) + '-[:OF {context:"' + contexts[indx].uid + '",user:"' + user.uid + '",timestamp:"'+(timestamp+index)+'"}]->s CREATE ' + convertName(concepts[index]) + '-[:AT {user:"' + user.uid + '",timestamp:"' + (timestamp+index) + '",statement:"' + statement.uid + '"}]->'+ 'c_' + contexts[indx].name +' ';
        }
    ...
    if (mentions.length > 1) {
        ...
        for (var index = 0; index < mentions.length; index++) {
            ...
            if (mentions_added.indexOf(mentions[index]) == -1) {

                createMentionsQuery += 'MERGE (' + convertMention(mentions[index]) + ':Concept ' + '{name:"' + mentions[index] + '"}) ON CREATE SET ' + convertMention(mentions[index]) + '.timestamp="' + (timestamp + index) + '", ' + convertMention(mentions[index])  + '.uid="' + node_uid + '" ';

                // Add the node we just added into another array, to check for duplicates in the next cycle
                mentions_added.push(mentions[index]);
            ...
            for (var indx = 0; indx < contexts.length; ++ indx) {
                ...
                createMentionsQuery += 'CREATE ' + convertMention(mentions[index]) + '-[:OF {context:"' + contexts[indx].uid + '",user:"' + user.uid + '",timestamp:"'+(timestamp+index)+'"}]->s CREATE ' + convertMention(mentions[index]) + '-[:AT {user:"' + user.uid + '",timestamp:"' + (timestamp+index) + '",statement:"' + statement.uid + '"}]->'+ 'c_' + contexts[indx].name +' ';
            }
    ...
    createNodesEdgesQuery = matchUser + createContexts + createStatement + createNodesQuery + createEdgesQuery + createMentionsQuery + ';';
