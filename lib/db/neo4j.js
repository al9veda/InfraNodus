...
function convertName(input) {
    ...
    output = transliteration.transliterate(input);

    // Get rid of the digits in text
...
CypherQuery.addStatement = function(user,concepts,statement,contexts,gapscan,callback) {
    ...
    createEdgesQuery = 'CREATE ' + convertName(concepts[0]) + '-[:BY {timestamp:"' + timestamp + '",statement:"' + statement.uid + '"}]->u ';
    ...
    for (var index = 1; index < concepts.length; ++ index) {
        ...
        createEdgesQuery += 'CREATE ' + convertName(concepts[index])  + '-[:BY {timestamp:"' +(timestamp+index)+ '",statement:"' + statement.uid + '"}]->u ';
