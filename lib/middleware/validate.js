...
function extractConcepts(statement) {
    ...
    // TODO
    // Remove stopwords
    // Bring words to their morphemes
    // Remove stopwords again
    // Get every single word out

    var concepts = 'a';

    // Convert them to lowercase
    for (var i = 0; i < concepts.length; i++) {
        if (!S(concepts[i]).isUpper()) {
            concepts[i] = S(concepts[i]).dasherize().chompLeft('-').camelize().s;
        }
        else {
...
exports.getHashtags = function(field) {
    ...
    return function(req,res,next) {
        ...
        // TODO add a setting that will define whether text/hashtag/all words are searched
        if (hashtags[0] == null) {
            var hashtags = extractConcepts(val);
        }
