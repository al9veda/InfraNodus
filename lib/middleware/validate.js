...
function extractMentions(statement) {
    ...
    for (var i = 0; i < mentions.length; i++) {
        if (!S(mentions[i]).isUpper()) {
            mentions[i] = S(mentions[i]).dasherize().chompLeft('-').camelize().s;
        }
        else {
...
exports.isToDelete = function(){
    return function(req, res, next){
        ...
        if (req.body.delete == 'delete' || req.body.submit == 'edit' || req.body.delete == 'delete context') {
        ...
        if (req.body.privacy == 'make public') {
                ...
                dbneo.cypherQuery(privacyquery, function(err, uid){
                    ...
                    if(err) {
                        ...
                        res.error('This context is now public.');
                        res.redirect('back');
                    }
...
exports.getContextForEntry = function(field) {
    ...
    return function(req,res,next) {
        ...
        if (addedContexts.indexOf(',') > 0) {
            ...
            res.error('Please, choose at least one context for your statement.');
            res.redirect('back');
        }
...
exports.getContextPrivacy = function(){
    ...
    return function(req, res, next){
        ...
        if (req.params.context) {
            ...
            var query = 'MATCH (u:User{name:"' + contextforuser + '"}), (ctx:Context{name:"' + req.params.context + '"}), ctx-[:BY]->u RETURN DISTINCT ctx.public;';
...
exports.getDefaultUser = function(){
    ...
    return function(req, res, next){
        ...
        // Do we have user ID we want to view in our URL?
            ...
            req.params.user = 'help';

            var query = 'MATCH (u:User{name:"help"}) RETURN u.uid';
