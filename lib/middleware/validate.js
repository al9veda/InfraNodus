...
exports.isToDelete = function(){
    return function(req, res, next){
        ...
        if (req.body.delete == 'delete' || req.body.submit == 'edit' || req.body.delete == 'delete list') {


            var delete_query = [];

            if (req.body.delete == 'delete list') {
                ...
                dbneo.cypherQuery(idQuery, function(err, uid){
                    ...
                    delete_query[1] = 'MATCH (ctx:Context{uid:"' + context_id + '"}), ' +
                                      'c-[at:AT]->ctx, s-[in:IN]->ctx, ctx-[by:BY]->u ' +
                                      'WITH c,at,s,in,ctx,by,u ' +
                                      'MATCH con-[byc:BY]->u WHERE byc.statement = s.uid ' +
                                      'DELETE at,in,s,byc';


                    console.log(delete_query);

                    deleteStatement(goBackContext);
...
exports.getContextForEntry = function(field) {
    ...
    return function(req,res,next) {
        ...
        that.getContextID(res.locals.user.uid, default_context, statements, function(result) {
            var contexts = result;
