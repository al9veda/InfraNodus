...
// Let's authenticate user using the username and password enetered
...
User.getByName = function(name, fn){
    ...
    // Let's get the ID from the user's name
    User.getId(name, function(err, id){
        if (err) return fn(err);

        // Got the ID? Let's get the user
        User.get(id, fn);
    });
};

// Get the ID from the user's name
