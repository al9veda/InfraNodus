...
User.prototype.update = function(fn){
    ...
    dbneo.insertNode({
        ...
        // 1. Take the whole bunch of the statements


        // 2. Extract the contexts from them
        // 3. Check if each context is in the DB and what its ID is. If none - create one (see lib/entry.js)

        for (var key in options.defaultstatements) {
            if (options.defaultstatements.hasOwnProperty(key)) {
                var req = {
                    body:  {
                        entry: {
                            body: options.defaultstatements[key]
                        }
                    },
                    internal: 1
                };
